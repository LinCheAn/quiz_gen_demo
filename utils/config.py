...
class AppConfig:
    ...
    inference_backend: str = os.getenv("INFERENCE_BACKEND", "vllm")
    ...
    summary_transformers_quantization: str | None = os.getenv("SUMMARY_TRANSFORMERS_QUANTIZATION")
    ...
    quiz_transformers_quantization: str | None = os.getenv("QUIZ_TRANSFORMERS_QUANTIZATION")
    ...
    def __post_init__(self) -> None:
        ...
        self.inference_backend = self.inference_backend.strip().lower() or "vllm"
        self.embedding_conda_env = self.embedding_conda_env.strip()
        self.summary_server_conda_env = self.summary_server_conda_env.strip()
        self.summary_transformers_quantization = (self.summary_transformers_quantization or "").strip() or None
        self.quiz_server_conda_env = self.quiz_server_conda_env.strip()
        self.quiz_transformers_quantization = (self.quiz_transformers_quantization or "").strip() or None
        ...
        if self.inference_backend not in {"vllm", "transformers"}:
