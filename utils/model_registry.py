...
from utils.schemas import ModelPreset, ModelSelectionSnapshot
...
class ModelRegistry:
    ...
    def _validate(self) -> None:
        if len(self._models_by_id) != len(self.models):
            raise ValueError(f"Duplicate model ids found in {self.path}")
        if self.defaults.summary_model_id not in self._models_by_id:
            raise ValueError(
                f"Default summary model `{self.defaults.summary_model_id}` not found in {self.path}"
            )
        if self.defaults.quiz_model_id not in self._models_by_id:
            raise ValueError(
    ...
    def resolve_selection(
        ...
        try:
            summary = self._models_by_id[resolved_summary_id]
        except KeyError as exc:
            raise ValueError(f"Unknown summary model id: {resolved_summary_id}") from exc
        try:
            quiz = self._models_by_id[resolved_quiz_id]
        except KeyError as exc:
...
def build_runtime_config(base_config: AppConfig, selection: ModelSelectionSnapshot) -> AppConfig:
    return base_config.copy_with_overrides(
        ...
        quiz_server_quantization=selection.quiz.quantization,
    )
