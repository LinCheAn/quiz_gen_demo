...
class AppRenderingTest(unittest.TestCase):
    ...
    def test_pipeline_status_tabs_are_present_in_order(self) -> None:
        ...
        parent_ids = {self._layout_parents(config)[step_tab["id"]], self._layout_parents(config)[intermediate_results["id"]], self._layout_parents(config)[run_metadata["id"]]}
    ...
    def test_rag_controls_are_outside_intermediate_results(self) -> None:
        ...
        self.assertEqual(child_ids, [parents[custom_keywords["id"]], current_auto_keywords["id"], run_rag_button["id"]])
